...
from src.geolink2oereb.lib.interfaces.oerebkrmtrsfr.v2_0.classes import OeREBKRM_V2_0_DokumentTyp
...
class OEREBlexSourceCustom(OEREBlexSource):
    ...
    def _get_document_title(self, document, current_file, language):
        ...
        # return {language: u'{title} ({file_name})'.format(title=document.title, file_name=current_file.title)} # file_name=current_file.title
        if document.doctype == 'decree':
            user_title = document.title + ' (' + current_file._title + ')'
        else:
            user_title = document.title

        return {language: u'{user_title}'.format(user_title=user_title)}
...
def create_document_source(source_config, theme_code, language,
    ...
    Args:
        ...
        theme_code (str):
        language (str):
        oereb_lex_document_source_class (pyramid_oereb.contrib.data_sources.oereblex.sources.document.OEREBlexSource):
...
def load(
    ...
    for record in result:
        new_code = get_document_type_code_by_extract_value(
            theme_code,
        ...
        new_code = get_law_status_code_by_extract_value(
            theme_code,
            record.law_status.code
        )
